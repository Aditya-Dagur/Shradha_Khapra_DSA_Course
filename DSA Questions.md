...
2\. Linear Search-Return the address of the Element(key) if it present in the  given array.

&#x20;     Approach=> Run a loop and check if key==arr\[i] then return index(i) at which it is present else return false.



3\. Product \& Sum: Return the sum \& Product of the given array Elements.

&#x20;     Approach: Make 2 Variable sum=0,product=1, Then run a loop over the array Elements and add them as sum+=arr\[i] \& prod\*=arr\[i];
